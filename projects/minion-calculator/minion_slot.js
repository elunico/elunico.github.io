...
function indexForItem(itemName, optionElement) {
    let idx = 0;
    for (let option of optionElement.options) {
...
function calculateNewSpeed(baseSpeed, fuel, up1, up2, addbonus) {
    // speedbonus_sum = total fraction of speed multipliers of all sources
    // i.e 0.25 (elava) + 0.3 (rabbit pet) = 0.55 total bus sum
    // bonus speed = base speed * (1 - (speedbonus_sum / (1 + speedbonus_sum)))
    let bonus1 = getFuelSpeedMultiplier(fuel);
    let speedSum = bonus1 + Number(addbonus);
    if (up1 == 'flycatcher')
        speedSum += 0.2;
    ...
    return baseSpeed * (1 - (speedSum / (1 + speedSum)));
}
...
class MinionSlot {
    ...
    editMinion() {
        ...
        document.getElementById('fuel').selectedIndex = indexForItem(this.fuel, document.getElementById('fuel'));
        document.getElementById('updrade1').selectedIndex = indexForItem(this.upgrade1, document.getElementById('updrade1'));
        document.getElementById('upgrade2').selectedIndex = indexForItem(this.upgrade2, document.getElementById('upgrade2'));
